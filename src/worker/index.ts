...
// regeneration; the intersection stays harmless afterwards. ADMIN_PASSWORD
// is a secret (wrangler secret put / .dev.vars) — the single password
// gating diary reads and all writes (diary + chord sheets).
type AppEnv = {
  Bindings: Env & { MODELS: R2Bucket; ADMIN_PASSWORD?: string }
}
...
// public; writes require the ADMIN_PASSWORD secret (sent as a Bearer token —
// the same password that unlocks Diary.app, so there's one password to
// remember).
...
app.use("/api/sheets/*", async (c, next) => {
  ...
  if (!c.env.ADMIN_PASSWORD || token === c.env.ADMIN_PASSWORD) return next()
...
// every method requires the ADMIN_PASSWORD secret (the Diary.app unlock
// prompt) as a Bearer token. Unset secret leaves the routes open — dev
// convenience, same trade-off as chord sheets.
